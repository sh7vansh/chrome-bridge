---
name: chrome-bridge
description: Control, inspect, and automate the user's live, active Google Chrome browser session via the stateful Python REPL runtime. MANDATORY whenever the user asks to view open tabs, read active pages, click, fill forms, navigate, extract browser data, or run browser workflows. Do NOT fallback to static HTTP fetching (`read_url_content` or `curl`) when live browser interaction is requested.
---
...
Instead of calling multiple disconnected tools, execute Python code via the unified REPL tool or runner:
`execute_python(code)` or running python scripts interacting with `chrome_sdk`.
