...
def test_extension_id_from_path(tmp_path):
    """Test Chromium-compatible unpacked extension ID generation."""
    from setup_host import extension_id_from_path, DEFAULT_EXTENSION_ID

    ext_dir = tmp_path / "my_extension"
    ext_dir.mkdir()

    ext_id = extension_id_from_path(ext_dir)
    assert len(ext_id) == 32
    assert all('a' <= c <= 'p' for c in ext_id)

    # Deterministic output for same path
    assert extension_id_from_path(ext_dir) == ext_id


def test_register_browser_manifests_dynamic_extension_id(tmp_path):
    """Test register_browser_manifests uses dynamic unpacked extension ID or default."""
    from pathlib import Path
    from setup_host import register_browser_manifests, extension_id_from_path, DEFAULT_EXTENSION_ID
    ...
    ext_dir = tmp_path / "custom_ext"
    ext_dir.mkdir()

    expected_id = extension_id_from_path(ext_dir)
    ...
    if sys.platform.startswith("linux"):
        manifest_file = home_dir / ".config" / "google-chrome" / "NativeMessagingHosts" / "com.chrome_bridge.native.json"
        assert manifest_file.exists()
        data = json.loads(manifest_file.read_text())
        assert data["allowed_origins"] == [f"chrome-extension://{expected_id}/"]
